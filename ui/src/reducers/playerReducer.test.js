...
describe('playerReducer', () => {
  ...
  describe('sound output device', () => {
    it('defaults to the browser output', () => {
      const result = playerReducer(undefined, { type: '@@INIT' })
      expect(result.outputDevice).toBe('browser')
    })

    it('switches the selected output', () => {
      const result = playerReducer(undefined, setOutputDevice('xiaoai'))
      expect(result.outputDevice).toBe('xiaoai')
    })

    it('falls back to the browser for empty selections', () => {
      const state = playerReducer(undefined, setOutputDevice('xiaoai'))
      const result = playerReducer(state, setOutputDevice(''))
      expect(result.outputDevice).toBe('browser')
    })

    it('preserves the output device when the queue is cleared', () => {
      const state = playerReducer(undefined, setOutputDevice('xiaoai'))
      const result = playerReducer(state, clearQueue())
      expect(result.outputDevice).toBe('xiaoai')

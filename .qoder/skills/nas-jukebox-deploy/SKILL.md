...
2. **打开总开关**：`[Jukebox] Enabled = true`（Docker 可用 `ND_JUKEBOX_ENABLED=true`）。
...
- **第三方 App 的音量键控制的是服务器本机**：Subsonic `jukeboxControl` 走的是**上游**
  `core/playback`（mpv），与本 fork 的网页输出无关。这不是部署错误，别在它上面排查。

...
func serveIndex(ds model.DataStore, fs fs.FS, shareInfo *model.Share) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		appConfig := map[string]any{
			...
			// The master switch alone enables the feature: with no outputs yet, the
			// admin UI must still be reachable so outputs can be added, and the
			// player-bar selector hides itself until a remote output exists.

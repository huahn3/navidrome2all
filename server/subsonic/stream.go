...
// StreamAlias serves /rest/stream/{id}.{ext} — some renderers (notably Xiaoai
// smart speakers driven through text directives) refuse URLs whose path does
// not end with an audio file extension. The ID is taken from the path and the
// request is then handled exactly like /rest/stream.
func (api *Router) StreamAlias(w http.ResponseWriter, r *http.Request) (*responses.Subsonic, error) {

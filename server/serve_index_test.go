...
var _ = Describe("serveIndex", func() {
	...
	It("exposes the jukebox flag", func() {
		outputs := []conf.JukeboxOutputDevice{{ID: "nas", Name: "NAS", Type: "mpd", Address: "localhost:6600"}}
		conf.Server.Jukebox.Enabled = true
		conf.Server.Jukebox.Outputs = outputs
		r := httptest.NewRequest("GET", "/index.html", nil)
		w := httptest.NewRecorder()
		serveIndex(ds, fs, nil)(w, r)
		Expect(extractAppConfig(w.Body.String())).To(HaveKeyWithValue("jukeboxEnabled", true))

		conf.Server.Jukebox.Enabled = false
		w = httptest.NewRecorder()
		serveIndex(ds, fs, nil)(w, r)
		Expect(extractAppConfig(w.Body.String())).To(HaveKeyWithValue("jukeboxEnabled", false))

		// Enabled without any configured output: the flag stays on so the admin
		// UI can manage outputs; the player-bar selector hides itself instead
		conf.Server.Jukebox.Enabled = true

...
type JukeboxOutputDevice struct {
	...
	Address  string
	Password string
	// PathFrom/PathTo optionally rewrite the media file path before handing it to MPD.
	// The path is relative to the library root (MusicFolder), so MPD's music_directory
	// just needs to point at the same folder; these only rename path segments.
	PathFrom string
	PathTo   string
	// Xiaomi speaker (Type = "xiaomi") specific options
	Token         string // 32-char hex device token, enables local miIO control
	DID           string // numeric device ID (optional with a token: learned from the miIO handshake)
...
func setViperDefaults() {
	...
	viper.SetDefault("jukebox.outputs", []JukeboxOutputDevice{})
